...
- (void)config {
    ...
    _line.backgroundColor = UIColor.app_lineColor;
    [self addSubview:_line];
    
    _label = [[UILabel alloc] init];
    _label.textColor = UIColor.app_titleColor;
...
- (void)setText:(NSString *)text {
    _text = text;
    if (_text.length > 0) {
        _line.backgroundColor = UIColor.app_titleColor;
    } else {
        _line.backgroundColor = UIColor.app_lineColor;
    }
    _label.text = text;
...
- (void)setShowCursor:(BOOL)showCursor {
    ...
    if (_showCursor == YES && showCursor == YES) { //重复开始, 那么,什么也不做
        ...
        _cursor.backgroundColor = UIColor.app_mainColor;
