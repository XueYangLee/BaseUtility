...
#pragma mark ---------- CustomProgressLayer ----------
@interface CustomProgressLayer : CALayer
...
- (void)setProgressWithAnimated:(BOOL)animated {
    ...
    if (animated) {
        ...
        self.imageLayer.frame = CGRectMake(0, 0, _progress * self.frame.size.width, self.frame.size.height);
...
- (void)drawInContext:(CGContextRef)ctx {
    CGRect rect = CGRectMake(0, 0, self.progress * self.frame.size.width, self.frame.size.height);
    if (self.trackImage) {   // trackImage
        CGContextScaleCTM(ctx, 1, -1);
        CGContextTranslateCTM(ctx, 0, -rect.size.height);
        CGContextDrawImage(ctx, CGRectMake(0, 0, self.frame.size.width, self.frame.size.height), self.trackImage.CGImage);
    }
...
- (void)layoutSublayers {
    ...
    self.textLayer.frame = (CGRect){0, (self.frame.size.height - size.height) / 2, self.frame.size.width, size.height};
...
- (void)setRoundedCorner:(NSNumber *)roundedCorner {
    ...
    self.imageLayer.cornerRadius = self.frame.size.height / 2.0;
...
- (CALayer *)imageLayer {
    if (!_imageLayer) {
        ...
        _imageLayer.frame = CGRectMake(0, 0, self.progress * self.frame.size.width, self.frame.size.height);
...
#pragma mark ---------- CustomProgressView ----------

@implementation CustomProgressView

+ (Class)layerClass {
    return [CustomProgressLayer class];
}
...
- (void)setProgress:(float)progress animated:(BOOL)animated {
    ...
    CustomProgressLayer *layer = (CustomProgressLayer *)self.layer;
    layer.progress = _progress;
    [(CustomProgressLayer *)self.layer setProgressWithAnimated:animated];
}
